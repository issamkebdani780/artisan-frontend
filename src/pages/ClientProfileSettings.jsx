...
const ClientProfileSettings = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage({ text: 'Profil mis √† jour !', type: 'success' });
      setTimeout(() => setMessage({ text: '', type: '' }), 5000);
    } catch (err) {
      setMessage({ text: err.message || 'Erreur mise √† jour.', type: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <ClientLayout title="Espace Client" subtitle="Param√®tres">
      <div className="flex-1 font-['Outfit',sans-serif] py-4 text-left max-w-6xl">
        <div className="mb-10 px-2 text-left">
          <h2 className="text-3xl font-black text-slate-900 tracking-tighter uppercase mb-2">Param√®tres de compte</h2>
          <p className="text-slate-500 font-bold max-w-xl leading-relaxed text-sm">G√©rez vos informations, votre localisation et votre s√©curit√© au m√™me endroit.</p>
        </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-8">
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Vos donn√©es de base</p>
                  </div>
                </div>
                ...
                <div className="pt-8 border-t border-slate-50 flex justify-start">
                  <button type="submit" disabled={loading} className="px-10 py-5 bg-primary text-white rounded-[20px] font-black uppercase text-xs tracking-widest hover:opacity-90 transition-all shadow-xl shadow-primary/20 active:scale-95">Mettre √† jour le profil</button>
                </div>
              </form>
            </div>
          )}
            ...
            <div className="bg-white rounded-4xl p-8 lg:p-12 border border-slate-100 shadow-[0_20px_50px_-20px_rgba(0,0,0,0.05)] animate-in fade-in slide-in-from-bottom-2 duration-500">
               <form onSubmit={handleSubmit} className="space-y-8">
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div>
                    <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight">Localisation G√©o</h3>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">R√©gion et Commune</p>
                  </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-3">
            ...
            <div className="space-y-10 animate-in fade-in slide-in-from-bottom-2 duration-500">
               <div className="bg-white rounded-4xl p-8 lg:p-12 border border-slate-100 shadow-[0_20px_50px_-20px_rgba(0,0,0,0.05)]">
                  <div className="flex items-center gap-4 mb-10">
                    ...
                    <div>
                      <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight">Acc√®s & Protection</h3>
                  ...
                  <form onSubmit={handlePasswordChange} className="space-y-8">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-left">
                      <div className="space-y-3">
                        ...
                        <input type="password" required value={passwordForm.current} onChange={(e) => setPasswordForm({...passwordForm, current: e.target.value})} className="w-full h-15 rounded-2xl border border-slate-100 bg-slate-50 focus:border-primary outline-none px-6 font-bold text-sm shadow-sm" placeholder="‚DA¢‚DA¢‚DA¢‚DA¢‚DA¢‚DA¢‚DA¢‚DA¢" />
                      </div>
                      <div className="space-y-3">
                        <label className="text-[11px] font-black uppercase tracking-widest text-slate-400 ml-1">Nouveau MDP</label>
                        <input type="password" required value={passwordForm.new} onChange={(e) => setPasswordForm({...passwordForm, new: e.target.value})} className="w-full h-15 rounded-2xl border border-slate-100 bg-slate-50 focus:border-primary outline-none px-6 font-bold text-sm shadow-sm" placeholder="‚DA¢‚DA¢‚DA¢‚DA¢‚DA¢‚DA¢‚DA¢‚DA¢" />
                      </div>
                      <div className="space-y-3">
                        <label className="text-[11px] font-black uppercase tracking-widest text-slate-400 ml-1">Confirmation</label>
                        <input type="password" required value={passwordForm.confirm} onChange={(e) => setPasswordForm({...passwordForm, confirm: e.target.value})} className="w-full h-15 rounded-2xl border border-slate-100 bg-slate-50 focus:border-primary outline-none px-6 font-bold text-sm shadow-sm" placeholder="‚DA¢‚DA¢‚DA¢‚DA¢‚DA¢‚DA¢‚DA¢‚DA¢" />
                      </div>
                    </div>
                    <div className="pt-8 border-t border-slate-50 flex justify-start">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl">
                  <div className="bg-red-50/20 rounded-4xl p-10 border border-red-100/50 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
                   ...
                   <div className="bg-slate-50 rounded-4xl p-10 border border-slate-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
                    ...
                    <button onClick={() => { if(window.confirm('üö® Supprimer ?')) { apiService.logout(); navigate('/'); } }} className="px-8 py-4 border-2 border-slate-200 text-slate-500 rounded-xl font-black uppercase text-[10px] tracking-widest hover:bg-red-600 hover:border-red-600 hover:text-white active:scale-95 transition-all">Supprimer</button>
                  </div>
                </div>
            </div>
          )}
