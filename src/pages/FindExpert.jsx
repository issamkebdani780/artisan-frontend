...
const FindExpert = () => {
  ...
  return (
    <div className="bg-white text-slate-900 min-h-screen transition-colors">
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {/* Hero Search */}
        <section className="mb-16">
          <div className="relative rounded-[40px] overflow-hidden bg-slate-50 p-8 md:p-20 border border-slate-100 transition-colors">
          ...
          <div className="flex-1">
            ...
            {loading ? (
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-2 gap-8">
                {[1, 2, 3, 4].map((i) => (
                  <div key={i} className="bg-slate-50 rounded-[40px] h-[400px] animate-pulse transition-colors"></div>
                ))}
              </div>
            ) : experts.length === 0 ? (
