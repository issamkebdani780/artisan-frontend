...
const ArtisanSettings = () => {
  ...
  const categoryGroups = [
    {
      ...
      options: ["Menuisier √©b√©niste", "Menuisier de chantier (coffreur)", "Presseur de bois", "D√©corateur bois int√©rieur", "Fabricant de portes et fen√™tres en bois"]
    },
    {
      group: "Ferronnerie et Soudure",
      options: ["Ferronnier d‚DAôart", "Soudeur (arc et argon)", "Ferronnier m√©tallier (portails et grilles)", "Chaudronnier industriel", "Soudeur carrosserie auto"]
    },
    {
      group: "Plomberie et R√©seaux",
      options: ["Plombier sanitaire", "Plombier chauffage central", "Monteur de r√©seaux de gaz", "Installateur tuyauterie cuivre et PER", "Plombier maintenance eau"]
    },
    {
      group: "√âlectricit√© et √ânergie",
      options: ["√âlectricien b√¢timent", "√âlectricien industriel", "Technicien solaire photovolta√Øque", "Tireur de c√¢bles et filerie", "R√©parateur tableaux √©lectriques"]
    },
    {
      group: "Peinture et Pl√¢tre",
      options: ["Peintre d√©corateur", "Peintre automobile", "Pl√¢trier staffeur", "Marbrier (pon√ßage et lustrage)", "Vernisseur sur bois"]
    },
    {
      group: "Ma√ßonnerie et Finitions",
      options: ["Ma√ßon (brique et ciment)", "Carreleur (fa√Øence et marbre)", "Cr√©pisseur (enduits traditionnels)", "Technicien isolation thermique et √©tanch√©it√©", "Ma√ßon r√©novation"]
    },
    {
      group: "M√©canique et Machines",
      options: ["M√©canicien automobile", "M√©canicien moto", "Technicien moteurs √©lectriques", "R√©parateur groupes √©lectrog√®nes et pompes", "M√©canicien agricole"]
    },
    {
      group: "Couture et Cuir",
      options: ["Tailleur homme", "Couturi√®re sur mesure (femme)", "Rapi√©ceur retouche", "Cordonnier (chaussures cuir)", "Maroquinier (sellerie et petite maroquinerie)"]
    },
    {
      group: "Verre et Miroiterie",
      options: ["Menuisier aluminium et verre", "Verrier (coupe verre tremp√©)", "Miroitier (pose miroirs et d√©cor verre)", "Vitrier automobile", "Souffleur de verre artisanal"]
    },
    {
      group: "M√©tiers alimentaires artisanaux",
      options: ["Boulanger traditionnel", "P√¢tissier (oriental et viennoiserie)", "Fromager artisanal", "Apiculteur (miel et d√©riv√©s)", "Artisan conserveur (pickles et bocaux)"]
    },
    {
      group: "Jardinage et Espaces Verts",
      options: ["Jardinier paysagiste", "Ouvrier en am√©nagement des espaces verts", "Technicien en entretien des jardins", "Technicien en irrigation goutte-√†-goutte et arrosage", "√âlagueur et tailleur d‚DAôarbres et palmiers"]
    }
  ];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage({ text: 'Profil PRO mis √† jour !', type: 'success' });
      setTimeout(() => setMessage({ text: '', type: '' }), 5000);
    } catch (err) {
      setMessage({ text: err.message || 'Erreur lors de la mise √† jour.', type: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handlePasswordChange = async (e) => {
    ...
    if (passwordForm.new !== passwordForm.confirm) {
      setMessage({ text: 'MDP diff√©rents', type: 'error' });
      return;
    }
    setLoading(true);
    try {
      ...
      if (result.error) {
        ...
        setMessage({ text: 'MDP mis √† jour !', type: 'success' });
  ...
  return (
    <ArtisanLayout title="Artisan PRO" subtitle="Param√®tres">
      <div className="flex-1 font-['Outfit',sans-serif] text-left max-w-6xl py-4">
        <div className="mb-10 px-4 text-left">
          <h2 className="text-3xl font-black text-slate-900 tracking-tighter uppercase mb-2">Param√®tres Expert</h2>
          <p className="text-slate-500 font-bold max-w-xl text-sm leading-relaxed">G√©rez votre visibilit√© et la s√©curit√© de votre compte professionnel.</p>
        </div>
        ...
        <input 
          ...
          onChange={async (e) => {
            ...
            try {
              ...
              setMessage({ text: 'Photo de profil mise √† jour !', type: 'success' });
            } catch (err) {
                ...
                <div className="flex flex-col lg:flex-row gap-12 items-start">
                  <div className="relative group shrink-0 mx-auto lg:mx-0">
                   ...
                   <div className="flex-1 w-full grid grid-cols-1 md:grid-cols-2 gap-8">
                      ...
                      <div className="space-y-2 text-left">
                        <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">T√©l√©phone</label>
                      ...
                      {/* Exp√©rience input moved to Expertise tab */}
                   </div>
                </div>
                <div className="pt-10 border-t border-slate-50 flex justify-start">
                  <button type="submit" disabled={loading} className="px-10 py-5 bg-secondary text-white rounded-[20px] font-black uppercase text-xs tracking-widest hover:opacity-90 transition-all shadow-xl shadow-secondary/20 active:scale-95">Mettre √† jour l'identit√©</button>
                </div>
               </form>
            </div>
          )}
               ...
               <form onSubmit={handleSubmit} className="space-y-10">
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left">
                  ...
                  <div className="bg-slate-50/50 rounded-4xl p-8 border border-slate-100 text-left">
                    <h4 className="text-xs font-black text-slate-900 uppercase tracking-widest mb-6 px-2">Sp√©cialit√©s Expert</h4>
                  ...
                  <div className="bg-slate-50/50 rounded-4xl p-8 border border-slate-100 text-left">
                    <h4 className="text-xs font-black text-slate-900 uppercase tracking-widest mb-6 px-2">Exp√©rience par Sp√©cialit√© (ans)</h4>
                    {form.specialty.length === 0 ? (
                      <p className="text-sm text-slate-400 font-bold px-2">S√©lectionnez d'abord au moins une sp√©cialit√© ci-dessus.</p>
                    ) : (
            ...
            <div className="space-y-10 animate-in fade-in slide-in-from-bottom-2 duration-500">
               <div className="bg-white rounded-4xl p-8 md:p-12 border border-slate-100 shadow-[0_20px_50px_-20px_rgba(0,0,0,0.05)]">
                  <div className="flex items-center gap-4 mb-10 text-slate-900 uppercase">
                    ...
                    <h3 className="text-xl font-black">Acc√®s & S√©curit√©</h3>
                  </div>
