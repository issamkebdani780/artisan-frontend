...
const ArtisanProjects = () => {
  ...
  return (
    <ArtisanLayout title="Khadamati PRO" subtitle="Gestion des projets">
      <div className="p-8 flex-1 flex flex-col overflow-y-auto font-['Outfit',sans-serif] bg-slate-50/20 transition-colors">
        ...
        {pendingDevis.length > 0 && (
          <section className="mb-12 bg-white rounded-[40px] p-10 border-2 border-secondary/20 shadow-2xl shadow-secondary/5 relative overflow-hidden group transition-colors">
                ...
                <div key={devis.id} className="bg-slate-50 p-8 rounded-[32px] border border-slate-100 space-y-6 hover:bg-white hover:border-secondary/30 transition-all group/card shadow-sm hover:shadow-xl">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <h4 className="mt-4 text-lg font-black text-slate-900 uppercase tracking-tight line-clamp-1 transition-colors">{devis.client_name}</h4>
                    </div>
                    <span className="text-xl font-black text-secondary">{devis.budget} DA</span>
                  </div>
                  <p className="text-sm text-slate-500 font-medium leading-relaxed italic border-l-2 border-secondary/30 pl-4 transition-colors">"{devis.description}"</p>
                  <div className="flex items-center justify-between pt-4 border-t border-slate-100 transition-colors">
                    <span className="text-xs font-black text-slate-400 uppercase tracking-tight flex items-center gap-2 transition-colors">
        ...
        <div className="bg-white rounded-[40px] border border-slate-100 shadow-2xl shadow-slate-200/40 overflow-hidden mb-12 flex flex-col transition-colors">
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/50 transition-colors">
                    ...
                    <tr key={project.id} className="hover:bg-slate-50/50 transition-all group">
                      <td className="px-10 py-8">
                        <div className="flex flex-col gap-1">
                          ...
                          <span className="text-[9px] font-black text-slate-300 uppercase tracking-widest bg-slate-50 border border-slate-100 rounded px-2 py-0.5 w-fit transition-colors">{project.type}</span>
                        </div>
                      </td>
                      <td className="px-10 py-8 text-xs font-black text-slate-600 uppercase tracking-tight transition-colors">{project.client_name}</td>
                      <td className="px-10 py-8">
                        <span className="text-xs font-black text-slate-500 uppercase transition-colors">
          ...
          <div className="px-10 py-6 bg-slate-50/50 border-t border-slate-50 flex items-center justify-between transition-colors">
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest transition-colors">Affichage de {filteredProjects.length} projets actifs</p>
