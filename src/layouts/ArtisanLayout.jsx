...
const ArtisanLayout = ({ children, title = "Mihnati PRO", subtitle = "Premium Plan", hideHeader = false }) => {
  ...
  return (
    <div className="bg-white dark:bg-background-dark font-['Outfit',sans-serif] text-slate-900 dark:text-slate-100 flex min-h-screen transition-colors duration-300">
        ...
        <div className="flex flex-col gap-10 p-4 lg:p-8">
              ...
              <div className="flex flex-col">
                <h1 className="text-lg lg:text-xl font-black tracking-tighter flex items-center gap-1 uppercase truncate overflow-hidden text-slate-900 dark:text-white">
                  ...
                  {Number(currentUser?.is_verified) === 1 ? (
                    ...
                    <span className="material-symbols-outlined text-slate-300 dark:text-slate-600 text-base" title="En attente de vérification">hourglass_top</span>
                  )}
                </h1>
                <p className={`${Number(currentUser?.is_verified) === 1 ? 'text-secondary' : Number(currentUser?.is_verified) === -1 ? 'text-red-500' : 'text-slate-400 dark:text-slate-500'} text-[10px] font-black uppercase tracking-widest truncate overflow-hidden`}>
          ...
          <nav className="flex flex-col gap-3 overflow-y-auto custom-scrollbar">
            {[
              ...
              if (item.isExternal) {
                return (
                  <a key={idx} href={item.path} target="_blank" rel="noreferrer" className={`${baseClasses} text-secondary bg-secondary/5 dark:bg-secondary/10 border border-secondary/10 hover:bg-secondary/10 shadow-sm transition-colors`}>
                    {content}
                  </a>
                );
              }
              ...
              return (
                <a key={idx} href={item.path || '#'} className={`${baseClasses} ${isActive ? 'bg-secondary text-white shadow-xl shadow-secondary/20' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-white/5 hover:text-slate-900 dark:hover:text-white group'}`}>
            ...
            <header className={`h-24 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-50 dark:border-white/5 px-8 flex items-center justify-between fixed ${headerLeft} left-0 right-0 top-0 z-50 shrink-0 transition-all duration-300 shadow-sm`}>
              <div className="flex items-center gap-6 flex-1">
                ...
                <div className={`relative w-full max-w-md transition-all duration-300 ${isCollapsed ? 'lg:max-w-xl' : 'lg:max-w-md'} hidden lg:block`}>
                  ...
                  <input type="text" placeholder="Rechercher projets, clients..." className="w-full pl-12 pr-6 h-12 bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-white/5 rounded-2xl focus:ring-2 focus:ring-secondary/20 focus:bg-white dark:focus:bg-slate-800 outline-none transition-all text-xs font-bold dark:text-white" />
                </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-4 pl-2">
                  <div className="text-right hidden sm:block">
                    <p className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-tight truncate max-w-[150px]">{currentUser?.name || 'Artisan'}</p>
                  ...
                  <div className="size-12 rounded-2xl bg-white dark:bg-slate-800 p-1 border border-slate-100 dark:border-white/10 shadow-xl shadow-slate-200/50 dark:shadow-none shrink-0 group hover:scale-105 transition-transform">
